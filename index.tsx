...
class GeminiService {
  ...
  async sendMessage(history: Message[], userInput: string): Promise<{ text: string; sources?: Source[]; isLocal?: boolean }> {
    try {
      ...
      // Ensure history starts with 'user' for Gemini turn requirements
      ...
      const text = response.text || "Portal sync hazy. Recalibrating Arizona sensors.";
  ...
  async generateTrailImage(trailName: string): Promise<string> {
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: { parts: [{ text: `Cinematic Arizona red rock vista: ${trailName}. High desert aesthetic.` }] },
...
const ChatInterface = forwardRef((props: { initialMessage?: string; className?: string }, ref) => {
  ...
  useEffect(() => {
    if (messages.length === 0) {
      setMessages([{ id: 'welcome-scout', role: 'assistant', content: initialMessage || "Scout Online. How can I assist?", timestamp: new Date() }]);
    }
  }, [initialMessage]);
  ...
  async function handleSend(overrideInput?: string) {
    ...
    setMessages(prev => {
      ...
      (async () => {
        setIsLoading(true);
        try {
          const response = await geminiService.sendMessage(updated, textToSend);
          setMessages(current => [...current, { id: (Date.now() + 1).toString(), role: 'assistant', content: response.text, timestamp: new Date(), sources: response.sources }]);
        } catch (err) {
          setMessages(current => [...current, { id: 'err-' + Date.now(), role: 'assistant', content: "Satellite sync hazy. Switching to high-desert backup...", timestamp: new Date() }]);
        } finally {
          setIsLoading(false);
        }
      })();
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 md:p-8 space-y-8 custom-scrollbar">
        {messages.map((msg) => (
          <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-in`}>
            <div className={`max-w-[90%] ${msg.role === 'user' ? 'bg-zinc-100 p-4 rounded-2xl' : ''}`}>
        ...
        {isLoading && <div className="text-[10px] font-black text-zinc-300 uppercase animate-pulse">Scouting high-desert nodes...</div>}
      </div>
      <div className="p-4 border-t border-zinc-100">
        <div className="relative max-w-4xl mx-auto">
          <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()} placeholder="Ask Scout about trail discovery or recovery..." className="w-full bg-zinc-50 border border-zinc-200 rounded-xl px-4 py-4 pr-12 outline-none focus:bg-white transition-all shadow-sm" />
    ...
    <div className="fixed bottom-8 right-8 z-[9999]">
      {isOpen && (
        <div className="mb-6 w-[360px] sm:w-[480px] h-[640px] shadow-2xl rounded-[32px] overflow-hidden border border-zinc-200 bg-white animate-in slide-in-from-bottom-8">
          <ChatInterface initialMessage="Scout Portal Active. Searching trail reports and SageSuite directory..." />
        </div>
      )}
...
const ArchivePage = () => {
  const issues = [
    ...
    { title: "Hydration Science for Runners", slug: "hydration-science-for-desert-runners" }
  ];
  return (
    <div className="pt-24 pb-32 max-w-6xl mx-auto px-6">
      ...
      <p className="text-xl text-zinc-500 serif-text italic mb-16">Connect back to the full catalog of newsletter articles on healthandtravels.beehiiv.com.</p>
          ...
          <div key={idx} className="border border-zinc-100 rounded-[40px] overflow-hidden bg-white shadow-sm hover:shadow-xl transition-all">
            <div className="aspect-video bg-zinc-50 flex items-center justify-center">
              {images[t.name] ? <img src={images[t.name]} className="w-full h-full object-cover" /> : (
                <button onClick={async () => { const url = await geminiService.generateTrailImage(t.name); setImages(p => ({...p, [t.name]: url})); }} className="text-[10px] font-black uppercase text-[#0d47a1]">Reveal Scout View</button>
              )}
            </div>
            <div className="p-10">
              <h3 className="text-3xl font-black mb-2 uppercase">{t.name}</h3>
              <p className="text-[#0d47a1] text-[10px] font-black uppercase mb-8">{t.loc}</p>
              <button onClick={() => navigate('/chat', { state: { initialQuery: `Tell me about ${t.name}.` } })} className="w-full py-5 bg-zinc-950 text-white rounded-2xl text-[11px] font-black uppercase tracking-[0.2em]">Access Intel</button>
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="flex-grow bg-[#fafafa] min-h-[calc(100vh-73px)] py-10 px-6">
      <div className="max-w-7xl mx-auto h-[800px] flex flex-col lg:flex-row gap-10">
        <div className="lg:w-1/4 p-8 bg-zinc-950 rounded-[40px] text-white shadow-xl">
          <h3 className="text-4xl font-black uppercase mb-4 tracking-tighter">Scout Portal</h3>
          <p className="text-zinc-400 serif-text italic text-lg mb-10">Real-time trail intelligence and SageSuite directory access.</p>
        </div>
        <div className="lg:w-3/4 bg-white border border-zinc-200 rounded-[48px] overflow-hidden shadow-2xl">
          <ChatInterface ref={chatRef} initialMessage="Scout Online. Monitoring high-desert nodes. What intel do you require?" />
        </div>
      </div>
    </div>
  );
...
const Hero = () => (
  <div className="bg-white pt-24 pb-24 border-b border-zinc-100 text-center px-6">
...
const App = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <footer className="p-12 text-center border-t border-zinc-50 text-[10px] font-black uppercase text-zinc-400 tracking-[0.3em]">© {new Date().getFullYear()} {BRAND_NAME} • Powered by SageSuite</footer>
    </div>
  );
};
