...
const ChatInterface = forwardRef((props: { initialMessage?: string; className?: string }, ref) => {
  ...
  async function handleSend(overrideInput?: string) {
    ...
    // Construct updated history immediately for the API call
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        id: 'err-' + Date.now(),
        role: 'assistant',
        content: "Satellite sync interrupted. Scout switched to high-desert backup protocols.",
        timestamp: new Date()
      }]);
    } finally {
      setIsLoading(false);
      ...
      <div className="p-4 bg-white border-t border-zinc-100">
        <div className="relative">
          <input
            ...
            placeholder="Ask Scout about Sedona, trails, or SageSuite..."
    ...
    <div className="bg-white pt-24 pb-24 border-b border-zinc-100 relative overflow-hidden">
      <div className="max-w-4xl mx-auto px-6 text-center">
        <div className="flex justify-center mb-12">
          <img src={LOGO_DATA_URL} alt={BRAND_NAME} className="w-36 h-36 md:w-44 md:h-44 object-contain animate-in fade-in duration-1000" />
        </div>
...
const ArchivePage: React.FC = () => {
  ...
  return (
    <div className="pt-24 pb-32 max-w-6xl mx-auto px-6">
      ...
      <p className="text-zinc-500 serif-text italic mb-12 text-xl">Browse past dispatches. All links lead to our newsletter vault on Beehiiv.</p>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
        {issues.map((issue, idx) => (
          <a key={idx} href={BEEHIIV_URL} target="_blank" rel="noopener noreferrer" className="group cursor-pointer block">
            <div className="aspect-[4/3] bg-zinc-100 rounded-2xl mb-6 overflow-hidden border border-zinc-200 transition-all group-hover:shadow-xl group-hover:border-blue-100 flex items-center justify-center p-8">
               <span className="text-xs font-black uppercase tracking-widest text-zinc-300">Issue #{150-idx} Preview</span>
            </div>
      ...
      <div className="mt-20 p-12 bg-zinc-50 rounded-3xl text-center border border-zinc-200">
         <h2 className="text-2xl font-black uppercase mb-4 tracking-tight">Access the Full Vault</h2>
         <a href={BEEHIIV_URL} target="_blank" rel="noopener noreferrer" className="inline-block bg-[#0d47a1] text-white px-10 py-4 rounded-xl font-black uppercase text-xs tracking-[0.2em] shadow-lg">View All Articles on Beehiiv</a>
      </div>
    </div>
  );
};
        ...
        {trails.map((t, idx) => (
          <div key={idx} className="flex flex-col border border-zinc-100 rounded-[32px] overflow-hidden bg-white shadow-sm hover:shadow-xl transition-all">
            ...
            <div className="p-8">
              ...
              <button onClick={() => navigate('/chat', { state: { initialQuery: `Tell me about ${t.name} trail conditions.` } })} className="w-full py-5 bg-zinc-950 text-white rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] hover:bg-[#0d47a1] transition-all">Access Intel</button>
            </div>
          </div>
        ))}
      </div>
...
const ChatPage: React.FC = () => {
  ...
  useEffect(() => {
    if (state?.initialQuery && chatRef.current) {
      const timer = setTimeout(() => chatRef.current?.sendMessage(state.initialQuery!), 500);
        ...
        <div className="lg:w-1/4 space-y-8">
          ...
          <div className="p-8 bg-white border border-zinc-200 rounded-[40px]">
             ...
             <ul className="space-y-6 text-xs font-bold text-zinc-500">
               ...
               <li onClick={() => chatRef.current?.sendMessage("Best trails for sunrise")} className="cursor-pointer hover:text-black">Sunrise Trail Intel</li>
             </ul>
          </div>
        </div>
