  ...
  useEffect(() => {
    if (messages.length === 0) {
      setMessages([{
        id: '1',
        role: 'assistant',
        content: initialMessage || "Scout Portal Online. How can I help your journey through the High Desert?",
        timestamp: new Date(),
      }]);
    }
  }, [initialMessage]);
  ...
  const handleSend = async (overrideInput?: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        id: 'error-' + Date.now(),
        role: 'assistant',
        content: "Scout connection interrupted. Recalibrating sensors... Please try your request again in a moment.",
        timestamp: new Date()
      }]);
    } finally {
      setIsLoading(false);
    ...
    <div className={`flex flex-col h-full bg-white ${className}`}>
      ...
      <div className="px-6 py-3 border-b border-zinc-100 flex items-center justify-between bg-zinc-50/50">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">SageSuite Sync Active</span>
        </div>
        <div className="text-[10px] font-black text-zinc-300 uppercase tracking-widest">Portal Scout v2.5</div>
      </div>
            ...
            <div className={`max-w-[88%] ${msg.role === 'user' ? 'bg-zinc-100 p-5 rounded-2xl rounded-tr-none shadow-sm' : ''}`}>
              ...
              {msg.sources && msg.sources.length > 0 && (
                <div className="mt-6 flex flex-wrap gap-2">
                  {msg.sources.map((s, i) => (
                    <a key={i} href={s.uri} target="_blank" className="inline-flex items-center gap-2 text-[9px] font-bold text-blue-600 bg-blue-50 border border-blue-100 px-3 py-1.5 rounded-lg hover:bg-blue-100 transition-colors uppercase tracking-tight">
      ...
      <div className="p-6 bg-white border-t border-zinc-100">
        <div className="max-w-4xl mx-auto relative">
          <input
            ...
            placeholder="Query Scout about trail intel or portal setup..."
        ...
        <p className="text-center mt-4 text-[9px] font-black text-zinc-300 uppercase tracking-[0.4em]">Health & Travels Journal Division</p>
      </div>
    </div>
  );
});
