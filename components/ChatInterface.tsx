  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        id: 'error',
        role: 'assistant',
        content: "Signal lost in the canyon. Let me try re-syncing with the portal...",
        timestamp: new Date()
      }]);
    } finally {
      setIsLoading(false);
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 md:p-10 space-y-10 bg-[#fafafa]/20 custom-scrollbar">
        {messages.map((msg) => (
          <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-in`}>
            <div className={`max-w-[88%] ${msg.role === 'user' ? 'bg-zinc-100 p-5 rounded-2xl rounded-tr-none text-zinc-700 font-medium' : ''}`}>
              {msg.role === 'assistant' && (
              ...
              {msg.type === 'lead-capture' && (
                <div className="mt-6 p-6 bg-[#0d47a1] rounded-2xl text-white shadow-2xl shadow-blue-900/20">
                  ...
                  <form onSubmit={(e) => { e.preventDefault(); setIsSubmittingLead(true); setTimeout(() => { setMessages(prev => [...prev, { id: 'succ', role: 'assistant', content: "Intel dispatch complete. Check your inbox for access credentials.", timestamp: new Date(), type: 'success' }]); setIsSubmittingLead(false); }, 1500); }} className="space-y-3">
                    <input 
              ...
              {msg.sources && msg.sources.length > 0 && (
                <div className="mt-6 flex flex-wrap gap-2">
                  ...
                  {msg.sources.map((s, i) => (
                    <a key={i} href={s.uri} target="_blank" className="inline-flex items-center gap-2 text-[10px] font-bold text-blue-600 bg-blue-50/80 px-3 py-1.5 rounded-lg hover:bg-blue-100 transition-all border border-blue-100">
        ...
        {isLoading && (
          <div className="flex items-center gap-2 px-2 text-[#0d47a1]">
      ...
      <div className="p-6 bg-white border-t border-zinc-100">
        <div className="max-w-4xl mx-auto flex items-center bg-zinc-50 border border-zinc-200 rounded-2xl px-5 py-2 focus-within:bg-white focus-within:ring-4 focus-within:ring-blue-50 transition-all">
          ...
          <button 
            ...
            className="ml-3 w-10 h-10 bg-[#0d47a1] text-white rounded-xl flex items-center justify-center hover:scale-105 transition-transform disabled:opacity-30"
          >
