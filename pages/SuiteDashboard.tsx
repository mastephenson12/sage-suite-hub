...
const WelcomeOverview = () => {
  ...
  useEffect(() => {
    const fetchBrief = async () => {
      try {
        const prompt = "Act as 'Scout'. Provide a 3-point high-density intelligence briefing for Arizona TODAY. 1. Weather/Hazards. 2. Trail Closures/Shuttles (Sedona focus). 3. One recovery wellness business recommendation. Bullet points, elite tone.";
    ...
    <div className="animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
        ...
        <div className="lg:col-span-2 space-y-8">
          <section className="bg-zinc-950 p-10 rounded-[40px] text-white shadow-2xl relative overflow-hidden group">
          ...
          {/* Expert Node Cards - Loaded Instantly */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="p-8 border border-zinc-100 rounded-[32px] bg-white hover:border-black transition-all group">
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-400 mb-4">Vetted Regional Intel</p>
              <h4 className="text-lg font-black uppercase mb-4 tracking-tighter">Arizona High Desert</h4>
              <ul className="text-[11px] text-zinc-500 space-y-3 font-medium uppercase tracking-wider">
                <li className="flex items-start gap-3">
                  <span className="w-1 h-1 bg-black rounded-full mt-1.5 shrink-0"></span>
                  <span>Red Rock Pass required for Sedona trailheads.</span>
                </li>
                <li className="flex items-start gap-3">
                  <span className="w-1 h-1 bg-black rounded-full mt-1.5 shrink-0"></span>
                  <span>Sedona Shuttle: Active Thurs-Sun (Mandatory for Soldier Pass).</span>
                </li>
                <li className="flex items-start gap-3">
                  <span className="w-1 h-1 bg-black rounded-full mt-1.5 shrink-0"></span>
                  <span>Snake Season: Maintain visual focus on trail center.</span>
                </li>
              </ul>
            </div>
            <div className="p-8 border border-zinc-100 rounded-[32px] bg-zinc-50 hover:bg-white hover:border-black transition-all group">
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-400 mb-4">Gear Readiness</p>
              <div className="space-y-4">
                <div className="flex flex-wrap gap-2">
                  {['UV-50 Layer', '3L Bladder', 'Salt Tabs', 'Offline Maps'].map(item => (
                    <span key={item} className="px-3 py-1.5 bg-white border border-zinc-200 rounded-lg text-[9px] font-black uppercase tracking-widest text-zinc-600">{item}</span>
                  ))}
                </div>
                <div className="pt-4 border-t border-zinc-200">
                  <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-1">Scout Recommendation</p>
                  <p className="text-xs font-bold uppercase tracking-tight">Swap cotton for merino tech-wool in canyon humidity.</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-6">
          <div className="bg-blue-50/50 p-8 rounded-[32px] border border-blue-100">
            <h4 className="text-[10px] font-black uppercase tracking-[0.4em] text-blue-800 mb-6">Discovery Pipelines</h4>
            <div className="space-y-6">
              {[
                ...
                <div key={brand.label}>
                  <div className="flex justify-between items-end mb-1">
                    <p className="text-[8px] font-black text-blue-600/50 uppercase tracking-widest">{brand.label}</p>
                    <p className="text-[9px] font-black text-blue-900 uppercase tracking-tight">{brand.val}</p>
                  </div>
          ...
          <div className="p-8 bg-zinc-50 border border-zinc-100 rounded-[32px]">
             <p className="text-[9px] font-black uppercase tracking-widest text-zinc-400 mb-4">Satellite Health</p>
             <div className="flex items-center gap-3">
               <div className="flex gap-1">
                 {[1,2,3,4,5].map(i => <div key={i} className={`w-1 h-3 rounded-full ${i <= 4 ? 'bg-blue-500' : 'bg-zinc-200'}`}></div>)}
               </div>
               <span className="text-[10px] font-black uppercase tracking-widest text-zinc-900">84% Signal Strength</span>
             </div>
          </div>
        </div>
      </div>
